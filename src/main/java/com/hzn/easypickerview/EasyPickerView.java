...
public class EasyPickerView extends View {
    ...
    private final static float DEFAULT_TOP_ACR_RADIUS = 15.0f;
    private final static float DEFAULT_BOTTOM_ARC_RADIUS = 15.0f;
    ...
    private double mTopArcRadius;
    private double mBottomArcRadius;
    ...
    public EasyPickerView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mTopArcRadius = a.getFloat(R.styleable.EasyPickerView_epvTopArcRadius, DEFAULT_TOP_ACR_RADIUS);
        mBottomArcRadius = a.getFloat(R.styleable.EasyPickerView_epvBottomArcRadius,DEFAULT_BOTTOM_ARC_RADIUS);
        mTopArcRadius = Math.toRadians(mTopArcRadius);
        mBottomArcRadius = Math.toRadians(mBottomArcRadius);
        a.recycle();
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (heightMode != MeasureSpec.EXACTLY) { // wrap_content
            height = contentHeight + getPaddingTop() + getPaddingBottom();
        } else {
            setPadding(getMaxPadding(height),0,0,0);
            setItemsPaddings(height);
        }
        ...
        if (widthMode != MeasureSpec.EXACTLY) { // wrap_content
            width = contentWidth;
        }
    ...
    private int getMaxPadding(int height){
        int maxPadding = (int) (Math.tan(mTopArcRadius)*height/2);
        return maxPadding;
    }

    private void setItemsPaddings(int height){
        if(mItemsPadding == null){
            mItemsPadding = new int[maxShowNum];
            int halfItemsSize = 0;

            if(maxShowNum%2 == 0){
                halfItemsSize = maxShowNum/2;
            } else {
                halfItemsSize = maxShowNum/2+1;
            }

            double itemHeight = (double) height/2/halfItemsSize;

            int padding;
            double currentHeight;
            int halfItemsSizeTemp = halfItemsSize -1;
            for(int i = 0;i<maxShowNum;i++){
                if(i>halfItemsSize -1){
                    currentHeight = halfItemsSizeTemp*itemHeight;
                    padding = (int) (Math.tan(mBottomArcRadius)*currentHeight);
                    mItemsPadding[i] = padding;
                    halfItemsSizeTemp --;

                } else {
                    currentHeight = (i+1)*itemHeight;
                    padding = (int) (Math.tan(mTopArcRadius)*currentHeight);
                    mItemsPadding[i] = padding;
                }
            }

            int [] temp = new int[maxShowNum+2];

            temp[0] = mItemsPadding[0]*2 - mItemsPadding[1];
            temp[6] = mItemsPadding[mItemsPadding.length-1]*2 - mItemsPadding[mItemsPadding.length-2];
            int position = 1;
            for(int i = 0 ;i<=mItemsPadding.length-1;i++){
                temp[position] = mItemsPadding[i];
                position++;
            }
        ...
        for (int item:mItemsPadding){
            Log.i("shiwan","item:"+item);
        }
        ...
        if (null != dataList && dataList.size() > 0) {
            ...
            for (int i = -half; i <= half; i++) {
                ...
                if (index >= 0 && index < size) {
                    ...
                    float drawX = cx - textWidth/2;

                    drawX = startX + mItemsPadding[i+half];

                    canvas.drawText(text, drawX, tempY - (tempFm.ascent + tempFm.descent) / 2, textPaint);
                }
            }
        }
    }
    ...
    public void setDataList(ArrayList<String> dataList) {
        ...
        textPaint.setTextSize(textSize*textMaxScale);
        // 更新maxTextWidth
        if (null != dataList && dataList.size() > 0) {
            int size = dataList.size();
